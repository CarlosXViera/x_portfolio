...
var _HexContainer = __webpack_require__(174);

var _HexContainer2 = _interopRequireDefault(_HexContainer);

var _Hexagons = __webpack_require__(175);

var _Hexagons2 = _interopRequireDefault(_Hexagons);

var _scream = __webpack_require__(97);
...
var App = function (_React$Component) {
	...
	function App(props) {
		...
		_this.scream = (0, _scream2.default)({
			width: {
		...
		_this.state = {
			orientation: _this.scream.getOrientation(),
			width: window.innerWidth,
			height: window.innerHeight
		};

		_this.scream.on('orientationchangeend', function () {
			var orientation = _this.scream.getOrientation();
			_this.setState({ orientation: orientation });
		});
	...
	_createClass(App, [{
		...
		value: function handleOrientation() {
			var _this2 = this;

			this.scream.on('orientationchangeend', function () {
				var orientation = _this2.scream.getOrientation();
				_this2.setState({ orientation: orientation });
			});
			console.log('darya');
		}
	}, {
		...
		value: function render() {
			return _react2.default.createElement(
				...
				_react2.default.createElement(
					_HexContainer2.default,
					{ onLayout: this.handleLayout, orientation: this.state.orientation },
					_react2.default.createElement(_Hexagons2.default, { orientation: this.state.orientation })
				)
			);
		}
	}]);
...
function HexContainer(_ref) {
	var orientation = _ref.orientation,
	    children = _ref.children;

	var viewBox = orientation === 'landscape' ? '0 0 ' + window.innerWidth + ' ' + window.innerHeight : '0 0 ' + window.innerHeight + ' ' + window.innerWidth;

	return _react2.default.createElement(
		'svg',
		{ className: 'hexcontainer', viewBox: viewBox },
		children
	);
}
...
var _d3Selection = __webpack_require__(60);

var _utils = __webpack_require__(176);

var _nodeUuid = __webpack_require__(95);
...
var Hexagons = function (_React$Component) {
	...
	function Hexagons(props) {
		...
		_this.onRef = function (ref) {
			_this.setState({
				...
				return _this.renderHexagons(_this.props.orientation);
			}
			//this.randomSpinTurn();
			);
		};

		_this.el = (0, _utils.importTemplates)(['originalHex']);

		_this.colors = ['#343838', '#005F6B', '#008C9E', '#00B4CC', '#00DFFC'];
	...
	_createClass(Hexagons, [{
		...
		key: 'generateData',
		value: function generateData(selection, width, height) {
			var hSize = 15;
			var hSpacing = 12.5;
			var vSpacing = 8.5;
			...
			var templateHexagon = this.el.originalHex;
			var hCollection = [];
			var count = 0;
		...
		value: function selectHexagons() {
			...
			function chooseHex(start, direction, times, arr) {
				...
				return { start: start, end: { y: y, x: x }, t: t };
			}
			...
			function remap(arr, cb) {
				...
				var removed = arr.splice(arr.length - 2, 2);
				var a = arr.map(cb);
				return a.concat(removed);
			}

			var hexArr = this.hexagonArray;
			var coords = {
			...
			var _loop = function _loop(i) {
				var t = [];
				var topLayer = isEven(o.y) ? { y: o.y - 1, x: o.x + 1 } : { y: o.y - 1, x: o.x },
				    level = i + 1,
				...
				for (var j = 0; j < layers; j++) {
					...
					if (isTopDone === false) {
						isTopDone = true;
						var sides = chooseHex(topLayer, 'left', topBtmAmount, hexArr);
						leftSide.push(sides.end);
						rightSide.push(sides.start);
						t = [].concat(_toConsumableArray(t), _toConsumableArray(sides.t));
					}

					if (isBtmDone === false) {
						isBtmDone = true;
						var _sides = chooseHex(bottomLayer, 'left', topBtmAmount, hexArr);
						leftSide.push(_sides.end);
						rightSide.push(_sides.start);
						t = [].concat(_toConsumableArray(t), _toConsumableArray(_sides.t));
					}
					if (isTopDone && isBtmDone) {
						if (level === 1) {
							var adjustedleft = adjust(o, 'l', 1);
							var adjustedright = adjust(o, 'r', 1);
							var left = chooseHex(adjustedleft, 'left', 1, hexArr).start;
							var right = chooseHex(adjustedright, 'right', 1, hexArr).start;
							rightSide.push(right);
							leftSide.push(left);
							t = [].concat(_toConsumableArray(t), [left, right]);
							totalLayers.push(t);
							break;
						}
						rightSide = remap(rightSide, function (obj) {
							if (typeof obj === 'undefined') return;
							var adjusted = adjust(obj, 'r', 1);
							if (adjusted.x > hexArr[0].length - 1) return;
							if (adjusted.y < 0 || adjusted.y > hexArr.length - 1) return;
							var sides = chooseHex(adjusted, 'right', 1, hexArr);
							t = [].concat(_toConsumableArray(t), _toConsumableArray(sides.t));
							return adjusted;
						});

						leftSide = remap(leftSide, function (obj) {
							if (typeof obj === 'undefined') return;

							var adjusted = adjust(obj, 'l', 1);
							if (adjusted.x > hexArr[0].length || adjusted.x < 0) return;
							if (adjusted.y < 0 || adjusted.y > hexArr.length - 2) return;
							var sides = chooseHex(adjusted, 'left', 1, hexArr);
							t = [].concat(_toConsumableArray(t), _toConsumableArray(sides.t));
							return adjusted;
						});
		...
		key: 'renderHexagons',
		value: function renderHexagons(props) {
			var _this2 = this;

			var all = this.state.g.selectAll('g.hex').remove();
			...
			this.layers.forEach(function (obj, i, a) {
				var total = a.length - 1;
				if (obj.length === 0) console.log('empty');
				obj.forEach(function (obj2, j, b) {
					var node = _this2.hexagonArray[obj2.y][obj2.x].node();
					var secondTotal = b.length - 1;

					var tl = new _gsap.TimelineMax();
					_this2.animation.push(tl);
						...
						tl.from(node.children[0], 2, {
							...
							onComplete: function onComplete() {
								...
								try {
									...
									for (var _iterator = _this2.animation[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
										var v = _step.value;
		...
		key: 'render',
		value: function render() {
			return _react2.default.createElement('g', { className: 'hexagons', ref: this.onRef });
		}
	}]);
				...
				this.check = function(init) {
					...
					if (missing === 0 && func) {
						...
						if (global) {
							...
							if (!hasModule && "function" === "function" && __webpack_require__(380)){ //AMD
