...
function HexContainer(_ref) {
	...
	var viewBox = orientation === 'landscape' ? '0 0 1600 900' : '0 0 900 1600';
...
var Hexagons = function (_React$Component) {
	...
	_createClass(Hexagons, [{
		...
		value: function generateData(selection, width, height) {
			...
			for (var vHexagons = 0; vHexagons < vAmount; vHexagons++) {
				...
				for (var hHexagons = 0; hHexagons < hAmount; hHexagons++) {
					...
					translatedHexagon = selection.append('g').attr('id', '' + vHexagons + hHexagons).attr('transform', 'translate(' + translate + ')');

					translatedHexagon.html(templateHexagon);

					translatedHexagon.select('polygon').attr('class', 'hexagon').on('click', function (d) {
						(0, _d3Selection.select)(this).attr('stroke', 'yellow');
					});
		...
		value: function selectHexagons() {
			...
			totalLayers[14].forEach(function (obj) {
		...
		value: function renderHexagons(props) {
			...
			console.log((0, _d3Selection.select)('svg').node().getBoundingClientRect().width);
			console.log(window.innerWidth);
			this.hexagonArray = this.generateData(this.state.g, 1600, 900);

			this.layers = this.selectHexagons();

			this.layers.forEach(function (obj, i) {
