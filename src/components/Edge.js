...
var Edge = function(vertex_1, vertex_2, graph, edge_id)
{
	var self = this;
	...
	this.create = function(preserve_state)
	{
		...
		d3.select("svg")
			.append("line")
			.attrs(this.attrs)
			.on("click", self.edgeClickDelete);

		var vertexDrawFunc;
		if (preserve_state === true) // For use with import and "play" mode.
		{
			vertexDrawFunc = function(vertex) { vertex.draw(vertex.state); };
		}
	...
	this.initialize = function(vertex_1, vertex_2, graph)
	{
		...
		graph.edges[this.attrs.id] = this;
	};
